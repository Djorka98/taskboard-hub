...
const normalizeUserKey = (userId) => (userId?.trim() ? userId : guestUserKey);
export const useBoardsStore = create()(persist((set, get) => ({
    ...
    setActiveUser: (userId) => {
        ...
        const nextSelected = state.selectedBoardIdByUser[key] && nextBoards.some((board) => board.id === state.selectedBoardIdByUser[key])
            ? state.selectedBoardIdByUser[key]
            : firstBoardId;
        set({
            activeUserId: key,
            boards: nextBoards,
    ...
    addBoard: ({ name, description }) => {
        ...
        const id = trimmedName
            ...
            .replace(/\s+/g, '-')
            .replace(/-+/g, '-');
        if (scopedBoards.some((board) => board.id === id))
            return;
        const board = {
            id,
            name: trimmedName,
    ...
    updateBoard: (boardId, { name, description }) => {
        ...
        const userKey = get().activeUserId;
        const scopedBoards = get().boardsByUser[userKey] ?? [];
        const nextBoards = scopedBoards.map((board) => board.id === boardId
            ? {
                ...board,
                name: trimmedName,
    ...
    onRehydrateStorage: () => (state) => {
        ...
        const selected = selectedBoardIdByUser[activeUserId] && currentBoards.some((board) => board.id === selectedBoardIdByUser[activeUserId])
            ? selectedBoardIdByUser[activeUserId]
            : firstBoardId;
