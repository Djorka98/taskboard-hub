...
const columnFromStatus: Record<TaskEntity['status'], BoardColumn> = {
  todo: 'To Do',
...
const statusFromColumn: Record<BoardColumn, TaskEntity['status']> = {
  Backlog: 'todo',
  'To Do': 'todo',
...
const findColumnTag = (tags: string[]) => tags.find((tag) => tag.startsWith(columnTagPrefix));
const findBoardTag = (tags: string[]) => tags.find((tag) => tag.startsWith(boardTagPrefix));

export const columnToTag = (column: BoardColumn) => `${columnTagPrefix}${column.toLowerCase().replace(/\s+/g, '-')}`;
export const boardToTag = (boardId: string) => `${boardTagPrefix}${boardId}`;

export const parseColumnTag = (tags: string[]): BoardColumn | null => {
  const columnTag = findColumnTag(tags);
  if (!columnTag) return null;

  const value = columnTag.replace(columnTagPrefix, '');
  if (value === 'backlog') return 'Backlog';
  if (value === 'to-do') return 'To Do';
...
export const parseBoardTag = (tags: string[]): string | null => {
  const boardTag = findBoardTag(tags);
  if (!boardTag) return null;
  return boardTag.replace(boardTagPrefix, '') || null;
};

const normalizePriority = (priority: TaskEntity['priority']): TaskItem['priority'] => {
  return priority;
};
...
export const toTaskCard = (task: TaskEntity, user: AuthUser): TaskItem => {
...
export const toTaskUpdatePayload = (task: TaskItem, column: BoardColumn) => {
  const persistedTags = [
    ...task.labels,
...
export const boardColumns: BoardColumn[] = ['To Do', 'In Progress', 'Review', 'Done'];

const boardColumnLabelKeys: Record<BoardColumn, string> = {
...
export const getBoardColumnLabel = (column: BoardColumn, t: (key: string) => string) => t(boardColumnLabelKeys[column]);
